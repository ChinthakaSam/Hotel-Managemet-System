...
class pdf extends TCPDF
{
	...
	public function Header(){
		...
		$this->Ln(2);
		$this->SetFont('helvetica', 'B', 15);
		$this->Cell(189,15, 'Report of Issued goods', 0,1,'C');
		$this->Ln(10);

		$this->SetFont('times','B',10);
		$this->SetFillColor(224,235,255);
		$this->Cell(80,5,'Item Name',1,0,'C',1);
		$this->Cell(15,5,'Quantity',1,0,'C',1);
		$this->Cell(25,5,'Measuring Unit',1,0,'C',1);
		$this->Cell(27,5,'Issued Date',1,0,'C',1);
		$this->Cell(35,5,'Department',1,0,'C',1);
		$this->SetFont('times','',9);
		...
		$dep = DB::getInstance()->query("SELECT * FROM inv_issued_items WHERE issued_date BETWEEN '$sdate' AND '$edate'");
                                        foreach ($dep->results() as $dep) {
                                            ...
                                            $deps = DB::getInstance()->query("SELECT * FROM department WHERE depId = $e");
                                        			foreach ($deps->results() as $deps) {
                                            			$f = $deps->depName;
                                        			}
                                        	$this->Ln(5);
											$this->Cell(80,5,$a,1,0,'C',1);
											$this->Cell(15,5,$b,1,0,'C',1);
											$this->Cell(25,5,$c,1,0,'C',1);
											$this->Cell(27,5,$d,1,0,'C',1);
											$this->Cell(35,5,$f,1,0,'C',1);
		...
		$this->Cell(50,5,'Report of issuings between',0,0,'C');
