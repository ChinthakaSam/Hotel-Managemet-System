...
$leave = DB::getInstance()->query("SELECT * FROM employeeleave WHERE MONTH(startDate)=MONTH(CURDATE()) AND YEAR(startDate)=YEAR(CURDATE()) AND status = 'Rejected' AND depId =$did");
?>
...
?>
    ...
    <!-- Right Panel -->
<?php
$leave = DB::getInstance()->query("SELECT * FROM employeeleave WHERE MONTH(startDate)=MONTH(CURDATE())+1 AND YEAR(startDate)=YEAR(CURDATE()) AND status = 'Rejected' AND depId =$did");
?>
